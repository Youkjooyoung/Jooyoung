/* addPurchase.js – 구매 등록 + 모달 확인 + 실시간 validation */
(function($, w, d) {
  'use strict'; 
  if (!$) return;
  ...
  $(function() {
    ...
    function validateForm() {
      ...
      if (!date) {
        ...
        var today = new Date(), selected = new Date(date);
    ...
    $('#btnSubmit').on('click', function(e) {
      ...
      if (result.valid) {
        ...
        // 첫 번째 오류 input으로 이동
        result.firstInvalid.focus()[0].scrollIntoView({ behavior: 'smooth', block: 'center' });
      }
    });
