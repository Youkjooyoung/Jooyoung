/*!
 * login.js — jQuery 3.6.1
 * - JSP와 100% 디커플링 (data-* 로 값 주입)
 * - 이미지 버튼 / 텍스트 버튼(#btnLoginSubmit) 모두 지원
 * - Enter 키 로그인, 중복 클릭 방지, CSRF 헤더 자동 첨부
 * - 성공 시 frames 갱신 / 프레임 없으면 index.jsp 이동
 * - 카카오 로그인(최상위창 리다이렉트)
 */
(function (w, d, $) {
  'use strict';
  ...
  $(function () {
    ...
    var SELECTOR = {
      ...
      btnLoginText: '#btnLoginSubmit',     // (텍스트 버튼이 있으면 자동 지원)
    ...
    function goFramesAfterLogin(userId) {
      // frames가 있으면 각 프레임 새로고침, 없으면 index.jsp로.
      try {
    ...
    $(SELECTOR.btnKakao).on('click', function () {
      ...
      var url = 'https://kauth.kakao.com/oauth/authorize'
              ...
              + '&response_type=code'
			  + '&prompt=login';
      w.top.location.href = url; // 반드시 최상위 창에서 이동
    });
