	...
	<div class="container two-col">
		...
		<section class="col-left">
			...
			<table class="form-table">
				...
				<tr>
					<th>가격</th>
					<td><fmt:formatNumber value="${purchase.purchaseProd.price}"
							type="number" /> 원</td>
				</tr>
				<tr>
					<th>상세설명</th>
					<td style="line-height: 1.6;">${purchase.purchaseProd.prodDetail}</td>
				</tr>
			</table>
				...
				<c:choose>
					<c:when test="${not empty productImages}">
						<c:forEach var="img" items="${productImages}" varStatus="st">
							<div
								class="img-box <c:if test='${st.index >= 6}'>is-hidden</c:if>"
		...
		<section class="col-right">
			...
			<table class="form-table">
				...
				<tr>
					<th>구매수량</th>
					<td><fmt:formatNumber value="${purchase.qty}" type="number" />
						개</td>
				</tr>
				<tr>
					<th>단가</th>
				...
				<tr>
					<th>총 금액</th>
					<td><c:choose>
							<c:when test="${purchase.paymentAmount ne 0}">
								<fmt:formatNumber value="${purchase.paymentAmount}"
									type="number" /> 원
            </c:when>
							<c:otherwise>
								<fmt:formatNumber
									value="${purchase.purchaseProd.price * purchase.qty}"
									type="number" /> 원
            </c:otherwise>
						</c:choose></td>
				</tr>
				<tr>
					<th>결제수단</th>
					<td><c:choose>
							<c:when test="${purchase.paymentOption == 'KAKAO'}">카카오페이</c:when>
							<c:when test="${purchase.paymentOption == 'CAR'}">카드</c:when>
							<c:when test="${purchase.paymentOption == 'CAS'}">현금</c:when>
							<c:otherwise>${purchase.paymentOption}</c:otherwise>
						</c:choose></td>
				</tr>
				<tr>
					<th>주문상태</th>
					<td><c:choose>
							<c:when test="${purchase.tranCode == '001'}">
								<span class="badge badge-green">주문완료</span>
							</c:when>
							<c:when test="${purchase.tranCode == '002'}">
								<span class="badge badge-green">배송중</span>
							</c:when>
							<c:when test="${purchase.tranCode == '003'}">
								<span class="badge badge-green">배송완료</span>
							</c:when>
							<c:when test="${purchase.tranCode == '004'}">
				...
				<tr>
					<th>연락처</th>
					<td>${purchase.formattedReceiverPhone}</td>
				</tr>
				<tr>
					<th>배송주소</th>
					<td>${purchase.zipcode}${purchase.divyAddr}<br />
					<c:out value="${purchase.addrDetail}" /></td>
				</tr>
				<tr>
					<th>요청사항</th>
					<td><c:out
							value="${empty purchase.divyRequest ? '요청사항이 없습니다.' : purchase.divyRequest}" /></td>
				</tr>
			</table>
			...
			<div class="btn-area">
				<c:if test="${purchase.tranCode == '001'}">
					<button type="button" class="btn-green" id="btnEdit"
						...
						data-url="${ctx}/purchase/updatePurchase?tranNo=${purchase.tranNo}">
						주문정보 수정</button>

					<button type="button" class="btn-gray" id="btnCancel"
						data-tranno="${purchase.tranNo}"
						data-url="${ctx}/purchase/updateTranCode?tranNo=${purchase.tranNo}&tranCode=004">
						구매취소</button>
				</c:if>
				...
				<c:if test="${purchase.tranCode == '002'}">
					<button type="button" class="btn-green" id="btnConfirm"
						...
						data-url="${ctx}/purchase/updateTranCode?tranNo=${purchase.tranNo}&tranCode=003&prodNo=${purchase.purchaseProd.prodNo}">
						물품 수령확인</button>
				</c:if>

				<c:if test="${purchase.tranCode == '003'}">
					<span class="text-green">구매가 완료되었습니다.</span>
				</c:if>
				<c:if test="${purchase.tranCode == '004'}">
					<span class="text-red">해당 주문은 취소되었습니다.</span>
				</c:if>
			</div>
		</section>
	</div>
